...
[![Platform](https://img.shields.io/badge/platform-Android%20%7C%20Windows%20%7C%20Linux-lightgrey)](#installation)
...
The phone app is a native **Android APK** built with [Capacitor](https://capacitorjs.com/). A custom Java plugin reads the hardware magnetometer at full hardware speed, throttled to the user-selected sample rate in JavaScript. Data streams over a plain WebSocket to the PC server on the same local network.
...
| PC server / dashboard  | Windows 10/11 or Ubuntu 20.04+ (x64), [Node.js](https://nodejs.org) v18+ |
| Both together          | Same local network                                   |
...
**Privacy.** The app collects no user data. Sensor readings are transmitted only over the local network to the PC server you control — nothing is sent to any external server, API, or cloud service. The GitHub Pages download page makes no network requests (enforced by `connect-src 'none'` in its Content Security Policy).
